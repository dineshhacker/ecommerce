...
<head>
  ...
  background: linear-gradient(#003d4d 60%, #00b8e6 120%, #00b8e6 60%, #00b8e6 120%); text-align:center">
			<h1 style="font-size:16px;color:white;font-family:Andika">YOUR PRODUCTS</h1>
...
<table class="gradienttable" style="margin:0 auto; padding:5px; width:100%;" >
			...
			$disp = mysqli_prepare($link,"SELECT productid,quantity,current_date FROM purchase  where faculty_id=? ;");
			mysqli_stmt_bind_param($disp, 's', $_SESSION["fkey"]);
				mysqli_stmt_execute($disp);
			mysqli_stmt_bind_result($disp, $productid,$quantity,$current_date);
